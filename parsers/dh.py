...
def parse_order(text):
    ...
    # Extract paid date and formatted date
    ...
    shipping_match = re.search(r'Shipping: (.+?) XMR', text)
    order['shipping_method'] = shipping_match.group(1).strip() if shipping_match else None

    # Order total extraction
    order_total_match = re.search(r'Order total: XMR [\d\.]+ \| USD ([\d\.]+)', text)
    order['order_total_usd'] = float(order_total_match.group(1)) if order_total_match else None
